...
public class Field {
	...
	private static final char DEFAULT_CELL_VALUE = ' '; // значение €чейки поумолчанию
	...
	public Field() {				// конструктор класса с дефолтным размером пол€		
		this(DEFAULT_FIELD_SIZE);
	}
	
	public Field (int size) {		//конструктор класса с предустановленным размером пол€
		fieldSize = size;
		field= new char[fieldSize][fieldSize];
	}
	
	Input input = new Input();		//инициализаци€ класса Input дл€ ввода координат
	
	public int getFieldSize() {		//получение размера пол€
		return fieldSize;
	}
	...
	char getCell(int i, int y){								//смотрим содержимое €чейки
		return field[i][y];
	}
